...
function ido.start(items, accept, title)
	...
	ido.window.opts = {
		...
		col = math.ceil(width * 0.3),
		width = math.ceil(width * 0.4),
		height = math.ceil(height * 0.6) - 3,
	}
	...
	ido.bind({
		...
		["<bs>"] = function()
			...
			if query == "" then
				...
				vim.api.nvim_buf_set_text(ido.buffer.query, 0, -2, 0, -1, false, {})
			end
		end,
	})
end)
...
ido.register("colorschemes", function()
	...
	ido.start(colors, function(color)
...
ido.register("git_files", function()
	...
	ido.start(vim.fn.systemlist("git ls-files --cached --others --exclude-standard"), function(file)
		vim.cmd("edit " .. file)
	end, "Git Files")
end)
...
ido.register("git_grep", function()
	...
	local matches = vim.fn.systemlist("git grep -inI --untracked " .. vim.fn.shellescape(query))
	...
	ido.start(matches, function(match)
		...
		local col = vim.api.nvim_buf_get_lines(0, match - 1, match, false)[1]:find(query)
